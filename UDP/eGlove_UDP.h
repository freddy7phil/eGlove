...
/*---------------Global Macros---------------*/
//Define the IMU used - MPU9150 or MPU9250
#define MPU9250
...
//UDP macros
#define SERVICE_ADDR "127.0.0.1" //Address
#define SERVICE_PORT 5040	 	 //Port number
#define BUFLEN 	     2048        //Length of buffer
...
//Device i.d. Register - (Values - MPU9150 - 0x68 and MPU9250 - 0x75)
...
//UDP structures and variables
struct sockaddr_in myaddr, remaddr;
int fd, slen = sizeof(remaddr);
char *server = SERVICE_ADDR;	//Change this to use a different server
char buf[BUFLEN];
...
//Structure to store UDP data
struct UDP_Data
{
	uint32_t ID; 		 //Unique identifier for each sensor
	uint32_t timestamp ; //Timestamp in ms since the first measurement
	double payload[3];   //Intrinsic euler angles in xyz
}pBuffer[SEN_COUNT];
...
//Timer Variables
uint32_t Timer;    //Variable to store timestamp for sensor
uint32_t delta_ms; //Variable to store timestamp for UDP
